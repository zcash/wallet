...
impl From<MigrateError> for Error {
    fn from(value: MigrateError) -> Self {
        match value {
            ...
            MigrateError::SeedNotAvailable => todo!(),
            MigrateError::MnemonicInvalid(_) => todo!(),
            MigrateError::NetworkMismatch { .. } => todo!(),
            MigrateError::NetworkNotSupported(_) => {
                Error::from(ErrorKind::Generic.context(fl!("err-migrate-regtest")))
            }
        ...
        MigrateError: From<C::Error>,
    {
        ...
        let chain_tip = if tip_height == 0 {
            ...
            Some(BlockHeight::try_from(tip_height).map_err(|e| {
                ErrorKind::Generic
                    .context(fl!("err-migrate-invalid-chain-data", err = e.to_string()))
            })?)
        };
            ...
            match (legacy_transparent_account_uuid, legacy_seed_data.as_ref()) {
                ...
                (None, Some((seed, _))) if !wallet.keys().is_empty() => {
                    ...
                    let (account, _) = db_data.import_account_hd(
                        ...
                        &SecretVec::new(mnemonic.to_seed("").to_vec()),
                        ZCASHD_LEGACY_ACCOUNT,
                        &wallet_birthday,
                        Some("zcashd_mnemonic"),
                    )?;
    ...
    async fn start(&self) -> Result<(), Error> {
        ...
        let wallet_path = if self.path.is_relative() {
            if let Some(datadir) = self.datadir.as_ref() {
                ...
                migrate_zcash_conf::zcashd_data_dir()
