...
use crate::{
    ...
    cli::StartCmd,
    components::{
        chain_state::ChainState, database::Database, json_rpc::JsonRpc, sync::WalletSync,
    },
    config::ZalletConfig,
    error::Error,
    prelude::*,
...
impl StartCmd {
    pub(crate) fn register_components(&self, components: &mut Vec<Box<dyn Component<ZalletApp>>>) {
        ...
        components.push(Box::new(ChainState::default()));
    ...
    async fn start(&self) -> Result<(), Error> {
        let (chain_indexer_task_handle, rpc_task_handle, wallet_sync_task_handle) = {
            ...
            (
                components
                    .get_downcast_mut::<ChainState>()
                    .expect("ChainState component is registered")
                    .serve_task
                    .take()
