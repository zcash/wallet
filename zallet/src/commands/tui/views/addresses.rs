...
fn render_list(app: &App, addresses: &[AddressEntry], frame: &mut Frame<'_>, area: Rect) {
    if addresses.is_empty() {
        let p = Paragraph::new("No addresses for this account yet. Press 'a' to derive one.")
...
fn render_detail(app: &App, addresses: &[AddressEntry], frame: &mut Frame<'_>, area: Rect) {
    let block = Block::default().borders(Borders::ALL).title(" Receive ");

    let Some(entry) = addresses.get(
        app.addresses_selected
            .min(addresses.len().saturating_sub(1)),
    ) else {
        let p = Paragraph::new("Select an address to view it.")
            .block(block)
            .style(Style::default().fg(Color::DarkGray))
            .wrap(Wrap { trim: true });
        frame.render_widget(p, area);
    ...
    match qr::render(&entry.address) {
        ...
        Some(_) => {
            lines.push(Line::from(Span::styled(
                "(enlarge the window to show the QR code)",
                Style::default().fg(Color::DarkGray),
            )));
        }
        None => {
            lines.push(Line::from(Span::styled(
                "(address too long to render as QR)",
                Style::default().fg(Color::DarkGray),
            )));
        }
    }
