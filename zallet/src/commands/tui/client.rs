...
use crate::components::json_rpc::EphemeralCredential;
...
impl fmt::Display for ClientError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ClientError::Build(e) => write!(f, "failed to build RPC client: {e}"),
            ClientError::Request(e) => write!(f, "RPC request failed: {e}"),
        }
    }
}
...
impl WalletClient {
    /// Connects to a self-hosted ephemeral server using its in-memory credential.
    ...
    /// Reveals the plaintext mnemonic phrase for the account's seed.
    pub(super) async fn export_mnemonic(&self, account_uuid: &str) -> CallResult<ExportMnemonic> {
        let mut params = ArrayParams::new();
        params
            .insert(json!(account_uuid))
            .map_err(|e| ClientError::Build(e.to_string()))?;
        self.request("z_exportmnemonic", params).await
    }
...
/// The revealed mnemonic phrase for an account.
#[derive(Clone, Debug, Deserialize)]
pub(super) struct ExportMnemonic {
    pub(super) seedfp: String,
