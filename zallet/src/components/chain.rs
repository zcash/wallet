...
use zaino_state::chain_index::types::BestChainLocation;
use zaino_state::chain_index::types::NonBestChainLocation;
use zaino_state::{
    BlockCacheConfig, ChainIndex, NodeBackedChainIndexSubscriber, NonfinalizedBlockCacheSnapshot,
    StatusType,
    chain_index::{NodeBackedChainIndex, NonFinalizedSnapshot, source::ValidatorConnector},
};
        ...
        let resolved_validator_address = match config.indexer.validator_address.as_deref() {
            ...
            None => {
                ...
                let default_port = match config.consensus.network() {
                    // Mainnet default RPC port for Zebra/zcashd
                    Network::Consensus(consensus::Network::MainNetwork) => 8232,
                    // Testnet/Regtest default RPC port for Zebra/zcashd
                    _ => 18232,
                };
    ...
    pub(crate) async fn get_transaction(&self, txid: TxId) -> Result<Option<ChainTx>, Error> {
            ...
            .map_err(|e| ErrorKind::Generic.context(e))?
        {
            ...
            (None, orphans) => match orphans.into_iter().next() {
                Some(NonBestChainLocation::Block(hash)) => (Some(BlockHash(hash.0)), None),
