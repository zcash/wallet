...
impl Chain {
        ...
        let validator_rpc_address =
            config
                ...
                .unwrap_or_else(|| match config.consensus.network() {
                    Network::Consensus(consensus::Network::MainNetwork) => "127.0.0.1:8232",
                    _ => "127.0.0.1:18232",
                });
    ...
    async fn subscribe(&self) -> Result<NodeBackedChainIndexSubscriber, Error> {
    ...
    pub(crate) async fn get_sapling_subtree_roots(
        ...
        self.subscriber
            .get_subtree_roots(zaino_state::chain_index::ShieldedPool::Sapling, 0, None)
            .await
            .map_err(|e| ErrorKind::Generic.context(e))?
            .into_iter()
    ...
    pub(crate) async fn get_orchard_subtree_roots(
        ...
        self.subscriber
            .get_subtree_roots(zaino_state::chain_index::ShieldedPool::Orchard, 0, None)
            .await
            .map_err(|e| ErrorKind::Generic.context(e))?
            .into_iter()
            .map(|(root_hash, end_height)| {
                Ok(CommitmentTreeRoot::from_parts(
                    BlockHeight::from_u32(end_height),
                    MerkleHashOrchard::from_bytes(&root_hash)
                        .expect("zaino should provide canonical encodings"),
                ))
            })
            .collect::<Result<Vec<_>, Error>>()
...
impl ChainView {
    ...
    pub(crate) async fn tree_state_as_of(
        ...
        let chain_state = if let Some(header) = self.get_block_header(height).await? {
            let (sapling_treestate, orchard_treestate) = self
                .chain
                .get_treestate(&header.hash().into())
                .await
                .map_err(|e| ErrorKind::Generic.context(e))?;
