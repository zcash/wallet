...
mod tests {
    ...
    fn rpc_parse_monetary_values() {
        ...
        assert!(matches!(zatoshis_from_value(&"-0.00000001".into()), Err(_)));
        ...
        assert!(matches!(zatoshis_from_value(&"1e-9".into()), Err(_))); //should fail
        assert!(matches!(zatoshis_from_value(&"0.000000019".into()), Err(_))); //should fail
        assert_eq!(zatoshis_from_value(&"0.00000001000000".into()), zat(1)); //should pass, cut trailing 0
        assert!(matches!(zatoshis_from_value(&"19e-9".into()), Err(_))); //should fail
        assert_eq!(zatoshis_from_value(&"0.19e-6".into()), zat(19)); //should pass, leading 0 is present

        assert!(matches!(
            zatoshis_from_value(&"92233720368.54775808".into()),
            Err(_)
        )); //overflow error
        assert!(matches!(zatoshis_from_value(&"1e+11".into()), Err(_))); //overflow error
        assert!(matches!(zatoshis_from_value(&"1e11".into()), Err(_))); //overflow error signless
        assert!(matches!(zatoshis_from_value(&"93e+9".into()), Err(_))); //overflow error
    }
