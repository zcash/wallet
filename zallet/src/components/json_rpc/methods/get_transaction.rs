...
use zaino_state::{fetch::FetchServiceSubscriber, indexer::LightWalletIndexer};
...
use crate::components::{
    ...
    json_rpc::{
        balance::{wtx_get_credit, wtx_get_debit, wtx_get_value_out, wtx_is_from_me},
        server::LegacyCode,
        utils::{parse_txid, value_from_zat_balance},
    },
};
...
pub(crate) async fn call(
    ...
    let (amount, fee) = {
        let credit = wtx_get_credit(wallet, &tx, as_of_height, filter).ok_or_else(|| {
        ...
        let debit = wtx_get_debit(wallet, &tx, filter).ok_or_else(|| {
            // TODO: Either ensure this matches zcashd, or pick something better.
        ...
        let fee = if wtx_is_from_me(wallet, &tx, filter).expect("already checked overflow") {
