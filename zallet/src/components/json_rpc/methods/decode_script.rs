...
pub(crate) struct ResultType {
    ...
    ///
    /// Omitted for scripts that don't require signatures.
    #[serde(rename = "reqSigs", skip_serializing_if = "Option::is_none")]
    req_sigs: Option<u8>,
    /// The addresses associated with this script.
    ///
    /// Empty for non-standard scripts and nulldata scripts.
...
pub(crate) fn call(params: &Network, hexstring: &str) -> Response {
    ...
    let code = Code(script_bytes.clone());
    let asm = to_zcashd_asm(&code.to_asm(false));

    let (kind, req_sigs, addresses) = detect_script_info(&script_bytes, params);

    let p2sh = calculate_p2sh_address(&script_bytes, params);
    ...
    Code(script_bytes.to_vec())
        ...
        .map(|script_kind| match script_kind {
            ...
            ScriptKind::PubKey { data } => {
                let addresses = pubkey_to_p2pkh_address(data.as_slice(), params)
                    .into_iter()
                    .collect();
                ("pubkey", Some(1), addresses)
            }
            ScriptKind::MultiSig { required, pubkeys } => {
                let addresses: Vec<String> = pubkeys
                    ...
                    .collect();
                ("multisig", Some(required), addresses)
            }
...
mod tests {
    ...
    fn decode_multisig_script() {
        ...
        assert!(result.addresses.iter().all(|a| a.starts_with("t1")));
