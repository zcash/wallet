...
struct Balance {
    ...
    /// Pending balance that is not spendable at the requested number of confirmations.
    ///
    /// Omitted if zero.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub(crate) fn call(
    ...
    let (total_spendable, total_pending, total_dust) =
        summary
            .account_balances()
            .values()
            .map(|account| {
                Ok((
                    account.spendable_value(),
                    (account.change_pending_confirmation() + account.value_pending_spendability())
                        .ok_or(LegacyCode::Database.with_static(
                            "Wallet database is corrupt: storing more than MAX_MONEY",
                        ))?,
                    account.uneconomic_value(),
                ))
            })
            .try_fold::<_, _, RpcResult<_>>(
                (Zatoshis::ZERO, Zatoshis::ZERO, Zatoshis::ZERO),
                |(acc_spendable, acc_pending, acc_dust), account: RpcResult<_>| {
                    let (spendable, pending, dust) = account?;
                    Ok((
                        (acc_spendable + spendable).ok_or(LegacyCode::Database.with_static(
                            "Wallet database is corrupt: storing more than MAX_MONEY",
                        ))?,
                        (acc_pending + pending).ok_or(LegacyCode::Database.with_static(
                            "Wallet database is corrupt: storing more than MAX_MONEY",
                        ))?,
                        (acc_dust + dust).ok_or(LegacyCode::Database.with_static(
                            "Wallet database is corrupt: storing more than MAX_MONEY",
                        ))?,
                    ))
                },
    ...
    Ok(Balances {
        ...
        total: balance(total_spendable, total_pending, total_dust),
    })
}
